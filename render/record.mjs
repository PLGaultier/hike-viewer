 ...
 * out/hike.mp4 — no visible tab, no babysitting.
 *
...
const reachable = async () => {
  try {
    const r = await fetch(`http://localhost:${PORT}/api/hike`);
...
rmSync(`${ROOT}/out/frames`, { recursive: true, force: true });
mkdirSync(`${ROOT}/out/frames`, { recursive: true });
...
const count = existsSync(`${ROOT}/out/frames`) ? readdirSync(`${ROOT}/out/frames`).length : 0;
...
await run('bash', ['render/to-mp4.sh'], { cwd: ROOT, env: { ...process.env, FPS: String(FPS) } });
